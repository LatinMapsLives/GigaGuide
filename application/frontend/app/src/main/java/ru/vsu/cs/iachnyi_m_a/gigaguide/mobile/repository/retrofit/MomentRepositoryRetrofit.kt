...
class MomentRepositoryRetrofit(private val momentAPI: MomentAPI) : MomentRepository {
    override suspend fun getSightMoments(sightId: Long, language: String): List<MomentInfo>? {
        var response: Response<List<MomentDTO>> = momentAPI.getAllMoments(sightId = sightId, lang = language).execute()
